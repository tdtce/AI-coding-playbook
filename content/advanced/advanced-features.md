...
При желании можно создать в своём проекте пайплайн, который по нажатию кнопки в PR будет делать код-ревью через Claude Code и добавлять комменты к коду. У нас это сделано через Azure Agent, который подключен к Claude Code и по API постит комментарии в пулл-реквесты.
