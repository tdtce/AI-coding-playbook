...
### Rule of Two для ИИ-агентов

Prompt injection необязательно совершается злым хакером. Для агента вредным недоверенным инпутом может стать ридми подключаемой библиотеки, сообщение в командном мессенджере, результат вызова MCP.

[Agents Rule of Two](https://ai.meta.com/blog/practical-ai-agent-security/) предлагает оценивать три свойства любого агента:

- **A — обрабатывает недоверенный ввод**
- **B — имеет доступ к чувствительным данным или системам:** секретам, приватному коду, внутренним API, продовым БД
- **C — может изменять состояние или взаимодействовать с внешними системами:** делать пуш, мёрдж и деплой, перезаписывать данные, отправлять сообщения или выполнять сетевые запросы

В автономном режиме согласно "правилу двух" агенту рекомендуется одновременно давать не более двух из этих свойств:

| Комбинация | Необходимое ограничение |
|---|---|
| A + B | Блокируем изменение состояния и внешнюю коммуникацию либо отдельно одобряем каждое такое действие |
| A + C | Не даём доступ к секретам и внутренним системам, запускаем агента в изолированной среде |
| B + C | Не передаём агенту никакого недоверенного внешнего инпута |
| A + B + C | Не разрешаем автономную работу: требуется подтверждение человеком или другая надёжная проверка |

Например, агент одновременно читает текст публичного issue в Гитхабе, имеет доступ к прод-кластеру и может выполнять изменяющие `kubectl`-команды. Тогда нужно либо убрать прод-доступ, либо оставить только read-only команды, либо требовать проверки конкретной команды и её цели пользователем.

Rule of Two снижает аварийный ущерб от prompt injection, но не защищает от обычных ошибок. Это дополнение, а не замена стандартных средств - типа принципа наименьших привилегий и sandbox.
...
Чтоб не вводить каждый раз эти команды, можно в `~/.bashrc` положить такую обёртку:
...
- **`--dangerously-skip-permissions`** (он же `--yolo` в Codex) — YOLO-режим, Claude Code может делать всё, что явно не запрещено (например, сэндбоксом)
- **Auto mode** — можно включить по `Shift + Tab`, отдельная модель решает, безопасно ли запускать команду
- **Скилл `/fewer-permission-prompts`** — помогает найти в истории всех чатов безопасные команды, которые можно добавить в исключения

Сейчас я использую по дефолту auto mode. В дополнение к нему хорошо заходит команда [/goal](https://code.claude.com/docs/en/goal) - авто-режим позволяет не одобрять каждое использование инструмента (например, поиск или Bash), а выставленная цель предотвращает лишние уточнения от Клода, он будет работать, пока не выполнит задачу.

![Пример вывода /fewer-permission-prompts](/images/security/fewer-permission-prompts.png)
