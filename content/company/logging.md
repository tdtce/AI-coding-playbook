...
У Claude Code есть встроенный OpenTelemetry-экспорт — метрики и события по протоколу OTLP отправляются в OTLP-приёмник. Обычно это OpenTelemetry Collector, который дальше разводит данные по бэкендам (Prometheus для метрик, Loki для логов, Grafana для визуализации и т.д.). Включить можно централизованно через энв-переменные в managed-настройках организации.
