...
## Harness engineering: готовим окружение для агента

Модель — только один из компонентов ИИ-агента. Всё окружение, которое направляет агента и задаёт ему рамки, даёт ему инструменты, помогает проверить результат, называют **harness**. В это окружение со звёздочкой можно включить и особенности проекта и окружения - документацию, структуру репозитория, доступные CLI и данные, настройки разрешений, тесты, линтеры и многое другое.

Полезно смотреть на механизмы harness по двум осям:

| Механизм | До выполнения | После выполнения |
|---|---|---|
| Детерминированный | Хуки, ограничения доступов | Тесты, линтеры, тайп-чекеры |
| Вероятностный | `CLAUDE.md`, скиллы, примеры | Ревью субагентом или другим харнессом |

Правило в `CLAUDE.md` снижает вероятность ошибки, но не гарантирует, что агент будет следовать этому правилу всегда. Если проблему можно определить детерминированно, то всегда лучше превратить правило в тест или иную проверку. Конечно, есть случаи, где формальная проверка невозможна или слишком дорога.

Чем больше свободы мы даём агенту, тем важнее становится хорошая, быстрая и точная обратная связь. Хороший харнесс позволяет агенту самостоятельно увидеть ошибку, исправить её и повторить проверку. Подробнее об этом можно почитать в статье [Harness engineering for coding agent users](https://martinfowler.com/articles/harness-engineering.html).
